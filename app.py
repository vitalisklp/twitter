...
def twitter_auth():
    ...
    user = User.load_from_db_by_screen_name(access_token['screen_name'][0])
    if not user:
        user = User(access_token['screen_name'][0], access_token['oauth_token'][0],
                    access_token['oauth_token_secret'][0], None)
        user.save_to_db()
